class ActionPlaceFBFCampfireIndoor: ActionSingleUseBase
{
	void ActionPlaceFBFCampfireIndoor()
	{
	...
	{	
		FBF_FireplaceBase fireplace_in_hands = FBF_FireplaceBase.Cast( action_data.m_MainItem );
		
		//replace fireplace with lambda
		FBFCampfireToIndoorsLambda lambda = new FBFCampfireToIndoorsLambda( fireplace_in_hands, "FBF_FireplaceIndoor", action_data.m_Player, action_data.m_Player.GetLastFirePoint(), action_data.m_Target.GetObject() );
...
class FBFCampfireToIndoorsLambda : TurnItemIntoItemLambda
{
	int 		m_FirePointIndex;
	vector 		m_SmokePosition;
	
	void FBFCampfireToIndoorsLambda( EntityAI old_item, string new_item_type, PlayerBase player, vector pos, Object target )
	{
		...
		vector smoke_point_pos = target.GetSelectionPositionMS( FBF_FireplaceIndoor.FIREPOINT_SMOKE_POSITION + m_FirePointIndex.ToString() );
	...
	override void CopyOldPropertiesToNew (notnull EntityAI old_item, EntityAI new_item)
	{
		...
		FBF_FireplaceIndoor fireplace_indoor = FBF_FireplaceIndoor.Cast( new_item );
		if ( fireplace_indoor )
		{
